...
namespace AdventCode.Day20
{
    public class Worker
    {
        ...
        private HashSet<Coordinate> _rooms = new HashSet<Coordinate>();
        ...
        public int Work1()
        {
            ...
            pathLen = CalculateBranch(_input, _current, 1, 0, out aux, out auxB);
            return pathLen;
        }
        ...
        public int Work2()
        {
            ...
            CalculateBranch(_input, _current, 1, 0, out aux, out auxB);
            return _counter;
        }

        private int CalculateBranch(string input, Coordinate current, int index, int currentLength, out int nextIndex, out bool hasNextBranch)
        {
            _current = current;
            hasNextBranch = false;
            int pathLen = 0;
            while(index < input.Length - 1)
            {
                if (input[index] == '(')
                {
                    ...
                    while (hasNext)
                    {
                        Coordinate startPoint = _current;
                        branches.Add(CalculateBranch(input, _current, ++index, pathLen + currentLength, out index, out hasNext));
                        _current = startPoint;
                    }

                    if (branches.Any(b => b == 0))
                    {
                        index++;
                        continue;
                    }
                ...
                else if (input[index] == ')' || index == input.Length - 1)
                {
                    hasNextBranch = false;
                    break;
                }
                else
                {
                    ...
                    {
                        case 'N':
                            newCoord = new Coordinate(_current.X, _current.Y - 1);
                            break;
                        case 'S':
                            newCoord = new Coordinate(_current.X, _current.Y + 1);
                            break;
                        case 'E':
                            newCoord = new Coordinate(_current.X + 1, _current.Y);
                            break;
                        case 'W':
                            newCoord = new Coordinate(_current.X - 1, _current.Y);
                            break;
                    ...
                    if (pathLen + currentLength >= 1000 && !_rooms.Contains(newCoord))
                    {
                        _rooms.Add(newCoord);
                        _counter++;
                    }
