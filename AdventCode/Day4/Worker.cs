...
namespace AdventCode.Day4
{
    public class Worker
    {
        ...
        { 
            ...
            {
                records.Add(new Record(split));
            }

            var shiftRecords = new List<List<Record>>();
            var orderedRecords = records.OrderBy(r => r.Time);

            List<Record> shift = null;
            foreach (var record in orderedRecords)
            {
                if (record.GuardBeginShift != 0)
                {
                    shift = new List<Record>();
                    shift.Add(record);
            ...
            var guardNumbers = new List<int>();

            foreach (var shiftRecord in shiftRecords)
            {
                Guard guard;
                if (!guardNumbers.Contains(shiftRecord[0].GuardBeginShift))
                {
                    guardNumbers.Add(shiftRecord[0].GuardBeginShift);
                    guard = new Guard(shiftRecord[0].GuardBeginShift);
                    _guards.Add(guard);
                }
                else
                    guard = _guards.First(g => g.Number == shiftRecord[0].GuardBeginShift);

                guard.ServiceDays.Add(new DayOfService(shiftRecord));
            }
        }

        public int Work1()
        {
            int maxMinsAsleep = _guards.Max(g => g.MinutesAsleep);
            var guard = _guards.First(g => g.MinutesAsleep == maxMinsAsleep);
            return guard.MostSleepingMinute * guard.Number;
        }

        public int Work2()
        {
            int maxCount = _guards.Select(g => g.MostSleepingMinuteCount).Max();
            var guard = _guards.First(g => g.MostSleepingMinuteCount == maxCount);
            return guard.MostSleepingMinute * guard.Number;
        }

        string _testInput = @"[1518-11-01 00:00] Guard #10 begins shift
...
[1518-11-05 00:55] wakes up";
        ...
        string _input = @"[1518-10-31 00:58] wakes up
